...
#     """
#     Computes a safe starting pose: at the X,Y centroid, and Z set to 'z_offset' above the object's topmost point.
#     Returns np.ndarray for both position and orientation.

#     Args:
#         points (np.ndarray): 3D point cloud (N x 3)
#         orientation (np.ndarray): Orientation vector (default is (0, 0, 0))
#         z_offset (float): Offset added to the Z-coordinate (default is 100)

#     Returns:
#         np.ndarray: The initial position (shape: (1, 1, 3))
#         np.ndarray: The orientation (shape: (1, 1, 3))
#     """
#     if points is None or len(points) == 0:
#         raise ValueError("❌ Point cloud is empty. Can't compute initial position.")
...
#     initial_pos = np.array([centroid_x, centroid_y, safe_z]).reshape((1, 1, 3))
#     orientation_3d = np.array(orientation, dtype=float).reshape((1, 1, 3))

#     return initial_pos, orientation_3d
...
#         # ✅ Use points_array (convex hull points projected)
...
#         # Build convex hull again if needed for visualization
...
#             if i % 10 == 0:
#                 trajectory_positions.append(points_array[i])
...
# def compute_cross_trajectory(initial_position, raw_points, line_spacing=0.01):
#     """
#     Generate a cross-shaped trajectory centered on the initial_position.
#     The cross consists of two lines (X and Y directions), each extending
#     1cm beyond the object's bounds on both sides.
...
#     Args:
#         initial_position (np.ndarray): 3D point (x, y, z)
#         raw_points (np.ndarray): 3D point cloud (N x 3)
#         line_spacing (float): spacing between points on the lines (in meters)
...
#     # Determine object width and length (X and Y spans)
#     min_x, max_x = np.min(raw_points[:, 0]), np.max(raw_points[:, 0])
#     min_y, max_y = np.min(raw_points[:, 1]), np.max(raw_points[:, 1])
...
if __name__ == "__main__":
    ...
    # visualization_points = [((0.51646136, 0.3492587184444438, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.09641554844444379, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.22283713344444378, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.3492587184444438, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.4756803034444438, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.6021018884444438, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.3492587184444438, 0.33301859292592584), (0.0, 45.0, 0.0)), ((0.51646136, 0.3492587184444438, 0.5061991079259258), (0.0, 45.0, 0.0)), ((0.51646136, 0.3492587184444438, 0.6793796229259257), (0.0, 45.0, 0.0)), ((0.15595819, 0.59816507, 0.39017197000000003), (0.0, 45.0, 0.0)), ((0.15595819, 0.59816507, 0.5598776), (0.0, 45.0, 0.0)), ((0.15595819, 0.55816507, 0.63765935), (0.0, 45.0, 0.0)), ((0.15595819, 0.5503219, 0.64320531), (0.0, 45.0, 0.0)), ((0.15595819, 0.5303219, 0.6502763699999999), (0.0, 45.0, 0.0)), ((0.15595819, 0.5203219, 0.6502763699999999), (0.0, 45.0, 0.0)), ((0.15595819, 0.2903219, 0.64320531), (0.0, 45.0, 0.0)), ((0.15595819, 0.1603219, 0.6361342400000001), (0.0, 45.0, 0.0)), ((0.15595819, 0.15247873, 0.63058828), (0.0, 45.0, 0.0)), ((0.15595819, 0.13247873, 0.60937508), (0.0, 45.0, 0.0)), ((0.15595819, 0.10247873, 0.5386643999999999), (0.0, 45.0, 0.0)), ((0.15595819, 0.10247873, 0.51038013), (0.0, 45.0, 0.0)), ((0.15595819, 0.16247873, 0.43259838), (0.0, 45.0, 0.0)), ((0.15595819, 0.18247872999999998, 0.41845624000000003), (0.0, 45.0, 0.0)), ((0.15595819, 0.2103219, 0.39876815), (0.0, 45.0, 0.0)), ((0.15595819, 0.3203219, 0.32805746999999996), (0.0, 45.0, 0.0)), ((0.15595819, 0.4203219, 0.31391534), (0.0, 45.0, 0.0)), ((0.15595819, 0.4403219, 0.31391534), (0.0, 45.0, 0.0)), ((0.15595819, 0.5403219, 0.34927068), (0.0, 45.0, 0.0)), ((0.15595819, 0.5703219, 0.36341281000000003), (0.0, 45.0, 0.0)), ((0.15595819, 0.5781650700000001, 0.36895877), (0.0, 45.0, 0.0))]
    rotated_visualization_points = rotation.apply(visualization_points)

    if visualization_points.shape[0] > 1:
        polyline = pv.lines_from_points(rotated_visualization_points)
        plotter2.add_mesh(polyline, color='blue', line_width=3, label="Trajectory Path")
        plotter2.add_points(rotated_visualization_points, color='red', point_size=8, label="Trajectory Points")
    ...
    rotate_subsampled_positions = rotation.apply(subsampled_positions)
    arrow_points = pv.PolyData(rotate_subsampled_positions)
    arrow_template = pv.Arrow(direction=rotation.apply(fixed_orientation), scale=arrow_length)
    glyphs = arrow_points.glyph(orient=False, scale=False, geom=arrow_template)
    rotated_cross_points = rotation.apply(cross_points.reshape(-1, 3))
    rotated_initial_point = rotation.apply(initial_point)

    plotter2.add_mesh(glyphs, color='green',label="EE Orientation")   
    plotter2.add_points(rotated_initial_point, color='black', point_size=10, label="Initial Pose")
    plotter2.add_points(rotated_cross_points.reshape(-1, 3), color='orange', point_size=3, label="Cross Trajectory")
