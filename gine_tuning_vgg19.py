...
import keras._tf_keras.keras.backend
import tqdm
import keras 
...
def custom_loss(y_true, y_pred):
    ...
    epsilon = keras._tf_keras.keras.backend.epsilon()  # To avoid division by zero
    # Calculate binary crossentropy
    bce = keras._tf_keras.keras.backend.binary_crossentropy(y_true, y_pred)
    
    # Calculate the additional penalty for false positives
    # You can adjust the factor as needed (e.g., 2.0)
    penalty =  keras._tf_keras.keras.backend.sum((1 - y_true) * tf.cast(y_pred > 0.5, y_pred.dtype))
    
    return bce + 2.5 * penalty  # Combine BCE with penalty
...
def main():
    ...
    train_dir = r"C:\Users\Aggelos\ECE-2017-\Diplomatiki\data\binary_tumor_existence\tumor_dataset.7\Brain_Tumor_Detection\New folder"
    ...
    print(f"Batch structure: {len(batch)}")  # Check how many elements are in the batch
    print(f"Image shape: {batch[0].shape}")  # Print shape of image data
    print(f"Label shape: {batch[1].shape}")  # Print shape of label data
    ...
    while epochs_completed < target_epochs:
        ...
        # Now, pass class_weight_dict to model.fit()
        history = model1.fit(
            train_generator,
